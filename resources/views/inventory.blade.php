    ...
    @if (count($retailerInfo->inventories) > 0)
        @foreach ($retailerInfo->inventories as $row)
            <tr>
                ...
                <td>{{$row->UnitPrice}}</td>
                <td>
                    <a class="btn btn-primary" href="inventory/{{$row->medicine->MedicineId}}/edit"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil" viewBox="0 0 16 16">
